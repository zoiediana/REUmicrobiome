...
ps.file <- file.path(output.dir,"ps_oysters_rdp.rds")
track.file <- file.path(output.dir,"track_oysters_rdp.rds")
seqtab.file <- file.path(output.dir,"seqtab_nochim_rdp.rds")
```
...
write_rds(seqtab.nochim,seqtab.file)
```
...
track <- filt.out %>%
  as.data.frame %>%
  rownames_to_column %>%
  mutate(rowname=str_replace(rowname, "_R1_001.fastq.gz","")) %>%
  separate(rowname, into = "rowname2", sep = "_") %>%
  rename(sample=rowname2, input=reads.in, filtered=reads.out)
...
```{r}
rdp.ref <- '/sharedspace/oysters/ref/rdp_train_set_16.fa.gz'
taxa <- assignTaxonomy(seqtab.nochim, rdp.ref, multithread=TRUE)
```
...
sd <- read_csv(oyster.sample.data) %>%
