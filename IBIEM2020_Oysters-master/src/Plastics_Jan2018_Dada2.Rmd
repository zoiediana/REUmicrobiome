...
Folder names:
```{r}
baseFolder <- "/sharedspace/oysters/plastics/rittschof/RawDataJan2018_4522"
excelFile <- "/sharedspace/oysters/plastics/rittschof/sample_sheet_order_4522.xls"

outputFiltered <- "/sharedspace/oysters/plastics/scratch/filtered_Jan2018"
output.dir <- "/sharedspace/oysters/plastics/output"
ps.file <- file.path(output.dir,"ps_plastics_Jan2018.rds")
track.file <- file.path(output.dir,"track_track_Jan2018.rds")
```

Read in the files:
```{r}
run.files <- list.files(baseFolder, pattern = "R1_001.fastq.gz")
sampleTable <- read_xls(excelFile, skip = 3)
```

```{r}
matchtable <- sampleTable[,c(1,16)]
...
sampleInfo %<>% 
  ...
  separate(sampleID, into = c("plasticType","dayNum"),sep = "-", remove = FALSE) 
```
