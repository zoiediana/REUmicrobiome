...
# DESCRIPTION OF DATA (from Josh):
This data set is a duplicated time series of the microbiomes on 7 different kinds of recycle plastics that were simultaneously  exposed for 70 days to running sea watet and then sampled on day 8, 16, 24, 32 and 70. The idea is to use the microbiomes as a tool to figure out the nature of the additives in plastics by comparing them to microbiomes growing on toxic coatings and other coatings in marine environments.  The initial question is: How do the communities change with time and can we tell the plastics apart?

## Data storage
Downloaded from dds into /sharedspace/oysters/plastics
Based on the Order_4860_Notes.rtf, it seems that "1st MiSeq run did not perform to specifications so they ran it a 2nd time and gave us both sets of data."  So will proceed with the data from Order 4860?
...
Folder names:
```{r}
baseFolder2 <- "/sharedspace/oysters/plastics/rittschof/Order4860/Dressman_4860_18052402/"
baseFolder1 <- "/sharedspace/oysters/plastics/rittschof/Order4860/Dressman_4860_180521/"

outputFiltered <- "/sharedspace/oysters/plastics/scratch/filtered"
output.dir <- "/sharedspace/oysters/plastics/output/"
```

Read in the files:
```{r}
run2.files <- list.files(baseFolder2, pattern = "R1_001.fastq.gz")
```

# SAMPLE TABLE
```{r}
dayNum <- as.numeric(str_extract(run2.files, "[0-9]+"))
plasticType <- (str_extract(run2.files, "[aA-zZ]+"))
sampleName <- paste(plasticType,dayNum, sep = "-")
R1.fileName <- run2.files
R2.fileName <- str_replace(run2.files,"R1","R2")
sampleInfo <- data.frame(sampleName, plasticType,dayNum,R1.fileName, R2.fileName)
sampleInfo %>% arrange(plasticType,dayNum)
```

# View quality profiles
```{r}
i<-5
plotQualityProfile(file.path(baseFolder2, sampleInfo$R1.fileName[i]))
plotQualityProfile(file.path(baseFolder2, sampleInfo$R2.fileName[i]))
```
...
fnFs <- file.path(baseFolder2,as.character(sampleInfo$R1.fileName))
fnRs <- file.path(baseFolder2,as.character(sampleInfo$R2.fileName))
```
...
```{r}
filt.out <- filterAndTrim(fwd = fnFs, filt = filtFs, rev = fnRs, filt.rev = filtRs, trimLeft=10, truncLen=c(200,200),
...
sampleInfo %>%
  select(sampleName,plasticType,dayNum) %>%
...
```{r}
ps.file <- file.path(output.dir,"ps_plastics1.rds")
